	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (functionError) {
				...
				if (
					functionError.message?.includes("non-2xx") ||
					functionError.message?.includes("status code") ||
					functionError.message?.includes("Edge Function returned")
				) {
				...
				if (
					functionError.message?.includes("500") ||
					functionError.message?.includes("Internal Server Error")
				) {
